...
import { Plus, AlertTriangle, PlayCircle, FileJson } from 'lucide-react';
  ...
  return (
    <div className="w-1/3 min-w-[300px] max-w-md bg-white border-r border-slate-200 flex flex-col z-0">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        ...
        <button type="button" onClick={onNew} className="flex items-center gap-2 text-blue-600 hover:bg-blue-50 px-3 py-1 rounded-lg text-sm font-bold">
          <Plus size={16}/> New
        </button>
      </div>
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {content.map(item => (
          <div 
            ...
            className={`
              p-2 rounded-xl border transition-all flex items-center gap-3 cursor-pointer group
              ${selectedId === item.id 
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[10px] text-slate-500 font-mono bg-slate-200 inline-block px-1.5 rounded">
                {item.phonemeDisplay}
              </div>
            </div>
        ...
        {content.length === 0 && (
          <div className="flex flex-col items-center justify-center h-64 text-slate-400 p-8 text-center">
            ...
            <p className="text-xs mt-2">Import a database or click "New" to create words.</p>
          </div>
        )}
      </div>
    </div>
